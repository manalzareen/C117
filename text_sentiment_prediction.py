...
def predict(text):
    ...
    if  text!="":
        ...
        test_pad=np.array(padded)
        
        predicted_class_label = np.argmax(model.predict(test_pad), axis=1)        
        print(predicted_class_label) 
        for k,v in emo_code_url.items():
            if v[0]==predicted_class_label:
                p_img_url = v[1]
                p_emo = k
        return p_emo,p_img_url        
